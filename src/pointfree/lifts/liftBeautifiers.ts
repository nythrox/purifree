...
export type Ap<T> = { [applicativeSymbol]: T }
type Get<
  num extends number,
...
export type GetBCurried<
  T extends (a: Ap<any>) => (b: Ap<any>) => any
> = Get<0, ReturnType<T>>
export type GetCCurried<
  T extends (
    a: Ap<any>
  ) => (b: Ap<any>) => (c: Ap<any>) => any
...
export type GetDCurried<
  T extends (
    ...
    b: Ap<any>
  ) => (c: Ap<any>) => (d: Ap<any>) => any
> = Get<0, ReturnType<ReturnType<ReturnType<T>>>>

export type GetReturn<
  T extends (...args: Ap<any>[]) => any
> = ReturnType<T>[applicativeSymbol]
